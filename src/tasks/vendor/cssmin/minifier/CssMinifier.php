...
class CssMinifier {
     ...
     * @param boolean $packed to pack
     * @return void
     */
    public function __construct($source = null, array $filters = null, array $plugins = null, $pack = true) {
        $this->pack = $pack;
        ...
        foreach($filters as $name => $config) {
            if($config !== false) {
                ...
                if(class_exists($class)) {
                    ...
                    CssMin::triggerError(
                        new CssError(__FILE__, __LINE__, __METHOD__ . ": The filter <code>" . $name . "</code> with the class name <code>" . $class . "</code> was not found"));
                }
            }
        }
        // Plugins
        foreach($plugins as $name => $config) {
            if($config !== false) {
                ...
                if(class_exists($class)) {
                    ...
                    CssMin::triggerError(
                        new CssError(__FILE__, __LINE__, __METHOD__ . ": The plugin <code>" . $name . "</code> with the class name <code>" . $class . "</code> was not found"));
                }
            }
        }
        // --
