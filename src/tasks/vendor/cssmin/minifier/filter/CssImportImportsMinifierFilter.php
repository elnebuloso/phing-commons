...
class CssImportImportsMinifierFilter extends aCssMinifierFilter {
    ...
    public function apply(array &$tokens) {
        if(!isset($this->configuration["BasePath"]) || !is_dir($this->configuration["BasePath"])) {
            CssMin::triggerError(
                new CssError(__FILE__, __LINE__, __METHOD__ . ": Base path <code>" . ($this->configuration["BasePath"] ? $this->configuration["BasePath"] : "null") . "</code> is not a directory"));
            return 0;
        }
        for($i = 0, $l = count($tokens); $i < $l; $i++) {
            if(get_class($tokens[$i]) === "CssAtImportToken") {
                if(!preg_match('/^https?:\/\//', $tokens[$i]->Import, $m)) {
                    $import = $this->configuration["BasePath"] . "/" . $tokens[$i]->Import;
                }
                else {
                    $import = $tokens[$i]->Import;
                }

                // Import file was not found/is not a file
                if(in_array($import, $this->imported)) {
                    CssMin::triggerError(
                        new CssError(__FILE__, __LINE__, __METHOD__ . ": Import file <code>" . $import . "</code> was already imported.", (string) $tokens[$i]));
                    $tokens[$i] = null;
                }
                // Import file already imported; remove this @import at-rule to prevent recursions
                elseif(!($content = file_get_contents($import))) {
                    CssMin::triggerError(
                        new CssError(__FILE__, __LINE__, __METHOD__ . ": Import file <code>" . $import . "</code> was not found.", (string) $tokens[$i]));
                }
                else {
                    $this->imported[] = $import;
                    $content = file_get_contents($import);
                    $parser = new CssParser($content, array(
                        'Url' => array(
                            'BasePath' => dirname($import)
                        )
                    ));
